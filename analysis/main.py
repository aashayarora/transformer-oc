...
def load_t5_data(root_file):
    """Load T5 data from ROOT file."""
    ...
    return {
        ...
        'sim_vy': sim_vy,
        'sim_vz': sim_vz,
        'sim_t5IdxAllFrac': sim_t5IdxAllFrac,
        't5_isFake': t5_isFake,
    }


def calculate_t5_metrics(t5_data, pt_bins, eta_bins, eta_cut=None, purity_threshold=0.75):
    """Calculate efficiency and fake rate for T5 reconstruction."""
    ...
    for evt in range(n_events):
        ...
        # Process T5 tracks for fake rate
...
def plot_efficiency_fake_rate(pt_bins, eta_bins, metrics_pt, metrics_eta, output_path, epsilon):
    ...
    for metric_name, var_name, centers, y_data, y_err, xlabel, ylabel, color, ylim in plots:
        ...
        hep.cms.label(llabel="Phase-2 Simulation Preliminary", data=False, rlabel=r"$t\bar{t}$ PU100, 14 TeV", ax=ax)
        ax.legend()
...
def plot_purity_duplicate(pt_bins, eta_bins, metrics_pt, metrics_eta, output_path, epsilon):
    ...
    plots = [
        ...
        ('duplicate_rate', 'eta', eta_centers, metrics_eta['duplicate_rate'], metrics_eta['duplicate_rate_err'], 'Simulated track $\eta$', 'Duplicate Rate', 'purple', (0, max(0.5, np.max(metrics_eta['duplicate_rate']) * 1.2))),
    ]
...
def plot_t5_comparison(pt_bins, eta_bins, model_metrics_pt, model_metrics_eta, t5_metrics, output_path, epsilon):
    """Plot comparison of transformer and T5 efficiency and fake rate."""
    pt_centers = (pt_bins[:-1] + pt_bins[1:]) / 2
    eta_centers = (eta_bins[:-1] + eta_bins[1:]) / 2
...
def main():
    ...
    argparser.add_argument('--t5-file', type=str, default="/home/users/aaarora/phys/tracking/lst/lstod/CMSSW_15_1_0_pre2/src/RecoTracker/LSTCore/standalone/output_pu100_all.root", help='Path to ROOT file with T5 data')
    ...
    if args.t5:
        print(f"\nLoading T5 data from: {args.t5_file}")
        t5_data = load_t5_data(args.t5_file)
