...
def row_splits_from_strict_batch(batch):
    ...
    Example:
        ...
        # row_splits: tensor([0, 3, 5, 8], dtype=torch.int64)
    ...
    return torch.cat([torch.tensor([0], dtype=torch.int64, device=batch.device), change_indices + 1, torch.tensor([len(batch)], dtype=torch.int64, device=batch.device)]).to(torch.int64)
...
def strict_batch_from_row_splits(row_splits):
    ...
    Example:
        row_splits = torch.tensor([0, 3, 5, 8], dtype=torch.int64)
    ...
    batch = torch.repeat_interleave(torch.arange(
        len(lengths), dtype=torch.int64, device=row_splits.device), lengths)
